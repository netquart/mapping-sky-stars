...
Celestial.display = function(config) {
  ...
  setClip(proj.clip);
d3.select("#toggle-one")[0].parentNode.onchange=showgrid;
console.log(d3.select("#toggle-one")[0])
d3.select("#toggle-one").on("change",showgrid);
  ...
  function load() {
    ...
    d3.json(path + "mw.json", function(error, json) {
      if (error) { 
        window.alert("Your Browser doesn't support dfile loading or the file doesn't exist. See readme.md");
...
cfg. lines.graticule.opacity=0;
cfg.lines.graticule.stroke="#ffffff";
redraw();
console.log(cfg);
}
cfg.lines.graticule.show=true;
...
console.log(d3.select("#toggle-one"));
d3.select(".toggle-group").on('click',func);
...
var settings = { 
  ...
  stars: {
    ...
    namestyle: { fill: "#ddddbb", font: "11px 'Palatino Linotype', Georgia, Times, 'Times Roman', serif", align: "center", baseline: "top" },
...
function form(cfg) {
  ...
  var ctrl = d3.select(".col-md-9.col-sm-9").append("div").attr("class", "ctrl");
...
// Other
d3.select("canvas").attr("style","border:solid 2px;border-radius:50%;border-color:white;");

function color1()
{
  
   d3.select("body").attr("style","background-color:#1a1e2a");
   d3.select("canvas").attr("style","border:solid 2px;border-radius:50%;border-color:white;");
   d3.select("#left_container").attr("style","background-color:black;border-top:30px solid rgb(225, 227, 228);border-right:25px solid rgb(225,227, 228);border-left:25px solid rgb(225, 227, 228);border-radius:1% 1% 0px 0px;padding:2rem 1rem 2rem 1rem;");
   
   d3.select("#changed-color").attr("style"," background: black;border:4px solid white;border-radius:1%;padding: 1rem;height:710px")
   d3.select("#header_text").attr("style","text-align: center;color:white;color:white;");
   d3.select("#header_msg").attr("style","text-align: center; font-size: 1.4rem;color:white;");
   d3.select("#header_msgr").attr("style","text-align: center; font-size: 16px;color:white;");
      d3.select("#header_msg2").attr("style","text-align: center; font-size: 16px;color:white;");
...
function color2()
{
  ...
  d3.select("body").attr("style","background-color:#1a1e2a");
  d3.select("canvas").attr("style","border:solid 2px;border-radius:50%;border-color:white;");
  d3.select("#left_container").attr("style","background: #000045;border-top:30px solid rgb(225, 227, 228);border-right:25px solid rgb(225,227, 228);border-left:25px solid rgb(225, 227, 228);border-radius:1% 1% 0px 0px;padding:2rem 1rem 2rem 1rem;");

  d3.select("#changed-color").attr("style"," background: #000045;border:4px solid white;border-radius:1%;padding: 1rem;height:710px")
  d3.select("#header_text").attr("style","text-align: center;color:white;color:white;");
  d3.select("#header_msg").attr("style","text-align: center; font-size: 1.4rem;color:white;");
  d3.select("#header_msgr").attr("style","text-align: center; font-size: 16px;color:white;");
...
{
  d3.select("body").attr("style","background-color:#1a1e2a");
  d3.select("canvas").attr("style","border:solid 2px;border-radius:50%;border-color:#1a1e2a;");
  d3.select("#left_container").attr("style","background:white;border-top:30px solid rgb(225, 227, 228);border-right:25px solid rgb(225,227, 228);border-left:25px solid rgb(225, 227, 228);border-radius:1% 1% 0px 0px;padding:2rem 1rem 2rem 1rem;");

  d3.select("#changed-color").attr("style"," background:white;border: 4px solid #1a1e2a;border-radius:1%;padding: 1rem;height:710px")
  d3.select("#header_text").attr("style","text-align: center;color:white;color:black;");
  d3.select("#header_msg").attr("style","text-align: center; font-size: 1.4rem;color:black;");
  d3.select("#header_msgr").attr("style","text-align: center; font-size: 16px;color:black;");
  
     d3.select("#header_msg2").attr("style","text-align: center; font-size: 16px;color:black;");


}
frm.append("label").attr("class","labelmar").html("Sky Map Background Color");
frm.append("label").attr("class","labelmar1").html("Grid");
...
//  col.append("input").attr("type", "checkbox").attr("id", "mw-show").property("checked", config.mw.show).on("change", apply);
  ...
  col.append("label").attr("for", "mw-style-fill").html(" ");
  col.append("input").attr("type","button").attr("class","color1").attr("editable","false").attr("id", "mw-style-fill").attr("title", "Milky Way color").on("click", color1);
//
...
//  col.append("br");
  ...
  col.append("label").attr("for", "background").html("          ");
  col.append("input").attr("type","button").attr("class","color2").attr("id", "background-fill").attr("title", "Background color").on("click", color2);
    
    col.append("label").attr("for", "border").html("           ");
  col.append("input").attr("type","button").attr("class","color3").attr("id", "border-fill").attr("title", "Border color").on("click", color3);
col.append("input").attr("type","checkbox").attr("id","toggle-one").attr("style","margin-left:64px;height: 32px !important;width: 6% !important;vertical-align: middle;").property("checked",true);
  ...
  function apply() {
    ...
    switch (src.type) {
      case "color": if (testColor(src) === false) return; 
        ...
        document.getElementById("border").style.borderColor=value;
  }
...
//  col.append("label").attr("title", "Location coordinates long/lat").attr("for", "lat").html("Where do you want to see the sky from?");
...
//  }
  ...
  col.append("label").attr("title", "Location").attr("for", "txtarea").attr("id", "where_do").attr("style", "margin-top:1%;margin-bottom:1%;").html(" Where You Would Like to See the Sky From:");
    
  col.append("input").attr("type", "text").attr("id", "txtarea").attr("name","txtarea").attr("title", "Location").on("change", go);
    
    
  col.append("label").attr("title", "Local date/time").attr("id","castm").attr("for", "datetime").attr("style", "margin-bottom:1%;margin-left:-2px;").html("Time of Your Special Moment:");
...
//    go(); 
//  });
  col.append("input").attr("type", "hidden").attr("id", "datetime").attr("name","datetime").attr("title", "Date and time").attr("value", dateFormat(date, zone))
  .on("click", showpick, true).on("input", function () { 
    
    this.value = dateFormat(date, zone); 
    if (!dtpick.isVisible()) showpick(); 
  });
  var value =dateFormat(date, zone);
   var str=value.split("-");
   var day=str[2].split(" ");
   var row=col.append("div").attr("class","todaydate").attr("id","today");
  
   row.append("input").attr("type","date").attr("id","formdate").on("change",function(){
    
   
 
    var datetime= this.value;
    var parent = document.getElementById("location");
    var child = document.getElementById("datetime");
    parent.removeChild(child);

    col.append("input").attr("type", "hidden").attr("id", "datetime").attr("name", "datetime").attr("title", "Date and time").attr("value", datetime+" 15:36:37 +0300");
    ...
    // row.append("input").attr("class","todayday").attr('value',day[0]);
    // row.append("input").attr("class","todaymonth").attr('value',str[1]);
    // row.append("input").attr("class","todayyear").attr('value',str[0]);
...
//  col.append("label").attr("title", "Show horizon marker").attr("for", "horizon-show").html(" Horizon marker");
  col.append("input").attr("type", "checkbox").attr("id", "horizon-show").attr("style","display:none;").property("checked", cfg.horizon.show).on("change", go);    
//  col.append("label").attr("title", "Show solar system objects").attr("for", "planets-show").html(" Planets, Sun & Moon");
  col.append("input").attr("type", "checkbox").attr("id", "planets-show").attr("style","display:none;").property("checked", cfg.planets.show).on("change", go);    
//  
